...
| `ffmpeg_profile` | `software_default` | Base FFmpeg profile name used by profile resolution |
...
| `traffic_channel_enabled` | `false` | Enable Simulated Traffic |
| `traffic_aspect_ratio` | `16:9` | Traffic output aspect ratio |
| `traffic_resolution` | `1280x720` | Traffic output resolution |
| `traffic_rotation_mode` | `admin_rotation` | Traffic city-selection/rotation mode |
| `traffic_rotation_seconds` | `120` | Seconds per Traffic city in rotation |
| `traffic_pack_size` | `10` | Requested Traffic city pack size |
| `traffic_pack` | `[]` | Selected Traffic city pack |
| `traffic_music_mode` | `none` | Traffic music mode |
| `traffic_music_loop` | `false` | Whether Traffic music loops |
| `traffic_music_single_file` | empty | Selected Traffic single-track filename |
| `traffic_music_playlist_files` | `[]` | Selected Traffic playlist filenames |
| `traffic_cities` | `[]` | Persisted Traffic city configuration/selection |
| `news_channel_enabled` | `false` | Enable News Now |
| `news_aspect_ratio` | `16:9` | News output aspect ratio |
| `news_resolution` | `1280x720` | News output resolution |
| `news_feed_urls` | `[]` | Configured News RSS/Atom feed URLs |
| `news_music_mode` | `none` | News music mode |
| `news_music_loop` | `false` | Whether News music loops |
| `news_music_single_file` | empty | Selected News single-track filename |
| `news_music_playlist_files` | `[]` | Selected News playlist filenames |
| `channel_mix_enabled` | `false` | Enable Channel Mix |
| `channel_mix_name` | `Channel Mix` | Export/display name for Channel Mix |
| `channel_mix_channels` | `[]` | Ordered Channel Mix source/duration configuration |
| `channel_mix_music_mode` | `none` | Channel Mix music override mode |
| `channel_mix_music_loop` | `false` | Whether Channel Mix override music loops |
| `channel_mix_music_single_file` | empty | Selected Channel Mix single-track filename |
| `channel_mix_music_playlist_files` | `[]` | Selected Channel Mix playlist filenames |
...
Preview transport detection is tied to the selected source, not to a global mode. Local files resolve to `file`. Network sources are classified as `hls` or `mpegts` using a bounded probe; an inconclusive result remains empty/unknown. A cached result is used only when `guide_preview_transport_source_key` still matches the selected source.

Detected transport controls the internal Preview processing path and pacing. It does **not** change the generated Guide output format. Selectable HLS versus MPEG-TS Guide/Virtual Channel output is not implemented in v1.4.0.

Guide Message rotation uses explicit blocks:
...
Blank lines inside a message are preserved as spacing and never split the rotation. Untagged text is accepted as one normal-duration message for compatibility.
...
| `group-title` | Parsed/stored channel group metadata; it is not a Guide filter |
